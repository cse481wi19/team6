...
class Head(object):
    ...
    def __init__(self):
        ...
        self.client_pan = actionlib.SimpleActionClient(PAN_TILT_ACTION_NAME, control_msgs.msg.FollowJointTrajectoryAction)
        self.client_pan.wait_for_server()
        self.client_point = actionlib.SimpleActionClient(LOOK_AT_ACTION_NAME, control_msgs.msg.PointHeadAction)
        self.client_point.wait_for_server()
    ...
    def look_at(self, frame_id, x, y, z):
        ...
        goal.min_duration = rospy.Duration(secs=1)

        self.client_point.send_goal(goal)
        self.client_point.wait_for_result()
        return self.client_point.get_result()
    ...
    def pan_tilt(self, pan, tilt):
        ...
        assert(pan >= Head.MIN_PAN and pan <= Head.MAX_PAN)
        assert(tilt >= Head.MIN_TILT and tilt <= Head.MAX_TILT)
        ...
        point.time_from_start = rospy.Duration(secs=PAN_TILT_TIME)
        ...
        self.client_pan.send_goal(goal)
        self.client_pan.wait_for_result()
        return self.client_pan.get_result()
